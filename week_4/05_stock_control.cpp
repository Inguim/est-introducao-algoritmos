...
int main() {
    int qttProducts;
    string name, nameBigger;
    int n1, n2, qtt, qttBigger = 0;

    ifstream inputFile("estoque.txt");
    inputFile >> qttProducts;

    while(qttProducts > 0) {
        inputFile >> name >> n1 >> n2;
        qtt = n1 - n2;

        cout << name << " " << qtt;
        if (qtt < 50) cout << " BAIXO ESTOQUE";
        if (qtt >= qttBigger) {
            nameBigger = name;
            qttBigger = qtt;
        }
    ...
    cout << nameBigger << " " << qttBigger << endl;
    inputFile.close();
